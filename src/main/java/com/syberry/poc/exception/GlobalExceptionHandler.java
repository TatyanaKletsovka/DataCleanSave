...
public class GlobalExceptionHandler extends Exception {
   ...
   * Returns an HTTP Forbidden (403) status code.
   *
   * @param ex the exception to handle
   * @return an HTTP response entity containing the errors
   */
   ...
   * Returns an HTTP Unprocessable Entity (422) status code.
   *
   * @param ex the exception to handle
   * @return an HTTP response entity containing the errors
   */
  ...
  public final ResponseEntity<Map<String, List<String>>> argumentExceptionHandler(Exception ex) {
    List<String> errors = Collections.singletonList(ex.getMessage());
